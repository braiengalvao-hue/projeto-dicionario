...
<script>
    // Script de bloqueio de renderização para evitar "piscada" branca
    ...
    <main class="scroll_content">
        ...
        <div class="tabs_container">
            <button class="tab_btn active">
                ...
                <span class="tab_text">Pendentes (0)</span>
            </button>
            <button class="tab_btn">
                <i class="material-icons">done_all</i>
                <span class="tab_text">Aprovadas (0)</span>
            </button>
            <button class="tab_btn">
                <i class="material-icons">block</i>
                <span class="tab_text">Rejeitadas (0)</span>
            </button>
        </div>
    ...
    <div class="fab_container">
        <a class="fab_button" href="adicionar_termos_admin.php" title="Novo Termo">
            <img src="assets/images/plus-lg_icon.svg" id="add" alt="Adicionar">
        </a>
        ...
        <button id="theme-toggle" class="fab_button" aria-label="Alternar tema">
            <img src="assets/images/moon_icon.svg" id="theme-icon" alt="Tema" width="24">
        </button>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('api/termos.php?contar_status=true')
                ...
                .then(res => {
                    if (res.success) {
                        ...
                        tabs[2].querySelector('.tab_text').innerText = `Rejeitadas (${totalReprovados})`;
                    }
                });
        }
        ...
        function renderizarSugestoes(termos) {
            ...
            termos.forEach(termo => {
                ...
                // LÓGICA DOS BOTÕES COM SPAN PARA MOBILE
                let botoesHTML = '';

                if (statusAtual !== 'aprovado') {
                    botoesHTML += `
                        <button class="btn_approve" onclick="alterarStatus(${termo.id_termo}, 'aprovado')" title="Aprovar">
                            <i class="material-icons">check_circle</i> <span>Aprovar</span>
                        </button>`;
                }

                botoesHTML += `
                    <button class="btn_edit_outline" onclick='abrirEdicao(${JSON.stringify(termo)})' title="Editar">
                        <i class="material-icons">edit</i> <span>Editar</span>
                    </button>`;

                if (statusAtual != 'reprovado') {
                    botoesHTML += `
                        <button class="btn_reject_outline" onclick="excluirTermo(${termo.id_termo})" title="Excluir">
                            <i class="material-icons">delete_forever</i> <span>Excluir</span>
                        </button>`;
                } 
        ...
        // Funções de ação (alterarStatus, excluirTermo, abrirEdicao) permanecem as mesmas
